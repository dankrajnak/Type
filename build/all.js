...
function () {
  ...
  */
  function Typewriter(texts, el) {
    ...
    _defineProperty(this, "_executeNextDelay", function () {
      if (_this._stopImmediately) {
        _this._stopImmediately = false;
      } else {
        _this._delaySequenceRunning = true;

        if (_this._delaySequence.length > 0) {
          var nextDelay = _this._delaySequence.shift();

          setTimeout(function () {
            nextDelay.function();
            window.requestAnimationFrame(_this._executeNextDelay);
          }, nextDelay.delay);
        } else {
          if (_this._play) {
            _this.typeNextText(_this._playParams.typeDelayBase, _this._playParams.typeDelayVariance);

            _this.pause(_this._playParams.pause, _this._playParams.pauseVariance);

            _this.deleteAllCharacters(_this._playParams.deleteDelayBase, _this._playParams.deleteDelayVariance);

            _this.pause(_this._playParams.pause, _this._playParams.pauseVariance);

            _this._executeNextDelay();
          } else {
            _this._delaySequenceRunning = false;
          }
        }
      }
    });
    ...
    this._texts = texts;
    this._el = el; //Probability that a character will be mistyped

    this.errorProbability = errorProbability; //Current text displayed in the element.

    this._currentText = el.innerHTML; //Used to manage delays in adding characters.

    this._delaySequence = [];
    this._delaySequenceRunning = false; //Length of currentText after all delays in the delay sequence run.

    this._lengthAfterDelay = 0; //Used in play() to rotate through texts

    this._play = false;
    this._stopImmediately = false; //Which text in this._texts we're on.

    this._textIndex = 0; //Text to type next (used for play and typeNextText)

    this._textToType = this._texts[0].split(""); //Constants
  ...
  _createClass(Typewriter, [{
    ...
    value: function deleteCharacter() {
      ...
      this._delay(function () {
        if (_this2._currentText.length == 0) return false;
        _this2._currentText = _this2._currentText.substr(0, _this2._currentText.length - 1);
    ...
    value: function typeCharacter() {
      ...
      this._lengthAfterDelay++; //Maybe make a mistake
      ...
      if (Math.random() <= this.errorProbability) {
        ...
        this._delay(function () {
          _this3._currentText = _this3._currentText.concat(mistake);
      ...
      this._delay(function () {
        _this3._currentText = _this3._currentText.concat(character);
    ...
    value: function typeNextText() {
      ...
      this._textToType.forEach(function (character) {
        _this4.typeCharacter(character, delayBase, delayVariance);
      });
